...
import {GetProducts} from '../../../httpRequests/customer/orders';
...
function handelFinishOrder() {
    addOrder({productList:order,customerId:state.user._id,address:state.user.address}).then((isAdded)=>{       
  ...
  return (
    <React.Fragment>
     ...
     <Grid container column>
         <Grid item  md={3}>
             <Cart order={order} onAdd={handelAdd} onRemove={handelRemove} onFinishOrder={handelFinishOrder} buttonTitle={'add order'}/>
              </Grid>
         <Grid item md={9}>
